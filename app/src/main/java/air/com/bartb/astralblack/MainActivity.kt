...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (prefs.getBoolean("activity_exec", false)) {
            Intent(this, AstralFilt::class.java).also { startActivity(it) }
            finish()
        } else {
            val exec = prefs.edit()
        ...
        AppsFlyerLib.getInstance()
            .init(ASTRALCONST.AF_DEV_KEY, viewModel.conversionDataListener, applicationContext)
        AppsFlyerLib.getInstance().start(this)
        bxgsdfqwdfqw(1500)
    }
    private fun toTestGrounds() {
        Intent(this, AstralFilt::class.java)
    ...
    private fun bxgsdfqwdfqw(timeInterval: Long): Job {
        return CoroutineScope(Dispatchers.IO).launch {
            while (NonCancellable.isActive) {
                val hawk1: String? = Hawk.get(ASTRALCONST.C1)
                if (hawk1 != null) {
